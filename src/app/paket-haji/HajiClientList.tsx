...
// ... Keep pure UI components intact
...
function PaketCard({ paket, onImageClick, isHistory }: { paket: Paket; onImageClick: (src: string, alt: string) => void; isHistory?: boolean }) {
    ...
    const seatLabel = totalSeat === null || totalSeat === 0 ? "Segera Tersedia" : `${totalSeat} Kursi Tersedia`;
    ...
    return (
        <div className={`bg-white rounded-[2rem] overflow-hidden flex flex-col border border-gray-100 shadow-sm transition-all duration-300 ${isHistory ? 'opacity-60 grayscale' : 'hover:shadow-2xl hover:-translate-y-1'}`}>
            <div className={`relative w-full aspect-[4/5] overflow-hidden cursor-zoom-in group/img`} onClick={() => onImageClick(imgSrc, paket.nama)}>
                ...
                <span className="absolute top-4 left-4 text-[10px] font-bold tracking-widest uppercase px-3 py-1.5 rounded-full text-white shadow-md" style={{ backgroundColor: isHistory ? "#6b7280" : "#c97d20" }}>
                ...
                <div className="flex flex-col gap-2.5">
                    ...
                    <div className="flex justify-between items-center text-xs">
                        <div className="flex items-center gap-2 text-gray-500">
                            <svg className="w-4 h-4 flex-shrink-0 text-amber-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {paket.maskapai && (
                        <div className="flex justify-between items-center text-xs">
                            <div className="flex items-center gap-2 text-gray-500">
                                <PlaneIcon className="w-4 h-4 flex-shrink-0 text-amber-700" />
                        ...
                        {!isHistory && (
                            <div className="flex items-center justify-between mt-2">
                                <span className="text-xs text-gray-400 flex items-center gap-1">
                                    <MdOutlineAirlineSeatReclineNormal className="w-4 h-4 text-gray-400" />
                                    Kursi Tersedia
                                </span>
...
export default function HajiClientList({ initialPakets }: { initialPakets: Paket[] }) {
        ...
        <>
            ...
            <div className="relative z-10 -mt-7 max-w-2xl mx-auto px-4">
                <div className="bg-white rounded-2xl shadow-2xl p-2 flex items-center gap-3 border border-gray-100">
                    <input type="text" placeholder="Cari paket haji..." value={search} onChange={e => setSearch(e.target.value)} className="flex-1 px-3 py-2 outline-none" />
                </div>
            </div>
                ...
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-4">
                    ...
                    {showHistory && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 mt-2">
                            {historyPackages.map(p => <PaketCard key={p.id} paket={p} isHistory onImageClick={(_, alt) => setLightbox({ src: p.image || FALLBACK_IMAGES.haji, alt })} />)}
                        </div>
                    )}
                </div>
            )}
